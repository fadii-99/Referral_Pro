...
const DeleteMemberModal: React.FC<Props> = ({ open, member, onClose }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // ✅ DRF view ke mutabiq: id query param me
      const res = await fetch(`${serverUrl}/auth/employees/?id=${encodeURIComponent(member.id)}`, {
        method: "DELETE",
        headers: {
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/30 backdrop-blur-[1px] p-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl relative">
        <div className="flex items-center justify-between px-6 pt-6">
          <h3 className="text-xl font-semibold text-primary-blue">Delete Member</h3>
          <button onClick={onClose} aria-label="Close" className="h-8 w-8 rounded-full bg-white border grid place-items-center">
        ...
        <div className="px-6 pb-6 pt-4">
          <div className="flex items-start gap-3 py-3">
            <span className="h-10 w-10 rounded-xl bg-rose-50 text-rose-600 grid place-items-center"><FiTrash2 /></span>
            <p className="text-sm text-gray-700">
              Do you want to delete <span className="font-semibold text-[#0b0d3b]">{member.name}</span>? This action cannot be undone.
            </p>
          </div>
