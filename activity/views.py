...
from referr.models import Referral  # Add this import


class ReferralActivityListView(APIView):
...
class ActivityListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Get all referrals for this company
            referral_data = Referral.objects.filter(company_id=user_id)
            
            # Extract all referral IDs
            referral_ids = referral_data.values_list('id', flat=True)
            
            # Get activity logs for ALL referrals of this company
            qs = ActivityLog.objects.filter(referral_id__in=referral_ids)
            ...
            model_fields = {f.name for f in ActivityLog._meta.get_fields()}
            if "created_at" in model_fields:
                qs = qs.order_by("-created_at")  # Most recent first for dashboard
            elif "timestamp" in model_fields:
                qs = qs.order_by("-timestamp")
            else:
                qs = qs.order_by("-id")
